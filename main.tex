...
\chapter{Experiment Design}\label{chap:experiment_design}
    ...
    The rest of this chapter is organized in two parts. Part A lays out how each of the five models were trained. It describes the datasets, the pretraining curriculum, the hyperparameter search procedures, and the reinforcement-learning algorithms and loss functions. Part B specifies how the models were evaluated, and frames the two research questions the evaluation answers: whether the kernel embedding is a faithful conditioning signal whose failures are predicted by its geometry (\textbf{RQ1}), and whether reinforcement-learning finetuning improves over cross-entropy finetuning, in correctness and in semantic flexibility (\textbf{RQ2}).
    ...
    The contrast provided by these two cases supplies the model with the examples needed to disambiguate two .

    \section{Curriculum Pretraining}\label{sec:curriculum_pretraining}
    % Objective: standard per-token cross-entropy (next-token NLL) conditioned on the embedding
    %   via cross-attention (Ch.4 architecture).
    % Schedule: 4 stages trained consecutively, each resuming from the previous final_model;
    %   per-stage peak LR 1e-4 -> 5e-5 -> 1e-5 -> 5e-6 (roughly order-of-magnitude decay, NOT a
    %   strict halving); within each stage warmup_ratio 0.05 then linear decay; 100-epoch cap;
    %   early stopping + best-model on eval_loss; effective batch 1024 (256 x 4 H100, DDP).
    % Optimizer: AdamW (beta 0.9/0.999, eps 1e-8), weight_decay 0.01 (a deliberate fixed choice,
    %   NOT the HF default of 0.0), grad-clip max_norm 1.0, dropout 0.1 (GPT-2 default).
    % Convergence (early-stopped on eval_loss):
    %   | stage (cum.) | peak LR | epoch cap | stopped @ | wall  |
    %   |   1 (0-2)    |  1e-4   |   100     |    35     |  7m   |
    %   |   2 (0-3)    |  5e-5   |   100     |    23     |  12m  |
    %   |   3 (0-4)    |  1e-5   |   100     |    31     |  52m  |
    %   |   4 (0-5)    |  5e-6   |   100     |    51     | 5h07m |
    Each stage is trained with the standard teacher-forced cross-entropy (next-token negative log-likelihood) objective, conditioned on the target formula's embedding through the cross-attention mechanism of Chapter~\ref{chap:kernel}. The four stages run consecutively, each resuming from the weights and optimiser state of the previous stage.
    ...
    % Frame as: HPO CONFIRMED the pilot defaults (it did not improve on them).
    % - LR pilot sweep over {5e-6, 1e-5, 5e-5, 1e-4, 5e-4} (stage 1, fixed dropout 0.1 / wd 0.01,
    %   judged on semantic distance): winner LR = 1e-4 (0.0112).
    % - Optuna, 20 trials, LR fixed at 1e-4, searching dropout in [0,0.2] (step 0.05) and
    %   weight_decay in [0.005,0.02]: best trial 0.0135 -- WORSE than the pilot -> dropout 0.1 /
    %   weight_decay 0.01 retained.
    % - Later-stage LRs (5e-5/1e-5/5e-6) were hand-set by decay, not separately swept.
    % - The per-lambda critic-LR sweep belongs with the GAE variant (6.4.3).
    ...
    % NOTE (verification): the Optuna trials actually ran at peak LR ~5e-4 (confirmed from the log),
    % not 1e-4; left unstated in the prose above per Mees' call. Optuna's best semantic distance was
    % 0.0135 (selected-trial objective 0.0208, at dropout 0.0 / wd ~0.010); either way > the 0.0112 pilot.
    
    \section{Reinforcement-Learning Finetuning}\label{sec:rl_finetuning}
    
    \subsection{Motivation and hypotheses}\label{sec:rl_motivation}
    % WHY RL helps the orthogonal cases (RQ1). ONE spine, four moves; lean on the Ch.2
    % preliminaries (cite, do not re-derive). Exposure bias = the why-RL-at-all backdrop only.
    %   M1  Conditioning bottleneck. ||V_sem|| <= ||W^V|| ||emb(phi)|| (Ch.4): conditioning
    %       strength is tethered to embedding magnitude. A small norm has two causes (Ch.5):
    %       honest low variance vs. high-variance/low-norm anchor UNDER-COVERAGE -- the latter
    %       is this section's target.
    %   M2  The reward is a higher-fidelity semantic channel. R = 1 - normalised Hamming reads
    %       the FULL satvec, not the lossy M-dim embedding, so it carries target semantics the
    %       embedding has discarded. It is a TRAINING-time signal, re-accessed through the
    %       embedding at inference -> RL recalibrates the model's RESPONSE to a weak signal; it
    %       does not bypass the bottleneck.
    %   M3  The spine: a discount policy miscalibrated on the orthogonal cell. The kernel couples
    %       magnitude with informativeness on purpose (Ch.5); the CE-trained decoder learns
    %       "small norm -> lean on the prior". Correct on low-variance, WRONG on high-variance/
    %       low-norm (rich formula, under-registered, signal still workable). The reward's pressure
    %       to use the weak signal is variance-proportional (an embedding-ignoring generation
    %       incurs expected Hamming p(1-q)+(1-p)q -> 0.5 at p=0.5, -> 0 at the extremes), so RL
    %       recalibrates exactly the cell CE under-serves. A partial, DECODER-SIDE remedy for the
    %       Ch.5 orthogonality limitation (not a kernel fix).
    %   M4  Hypotheses (both tested in 6.8/6.9):
    %       H0 (premise): the model conditions on magnitude -> correct ~ emb_norm positive on
    %          CE-base. If flat, the spine is moot (and that itself is a finding -- see 6.8.3).
    %       H1 (primary): RL's benefit is concentrated in the high-variance/low-norm cell, not
    %          uniform.
    %   ASIDE (subordinate, theoretical, NOT a tested hypothesis; place last): the finetuning data
    %   shapes the reward landscape -- equivalents and near-misses are hard positives/negatives that
    %   sharpen the reward gradient around each target (6.1.2). The CE-finetuned control absorbs the
    %   data-side effects, so RL-vs-control reflects the objective, not the extra data.
    ...
    Reinforcement learning supplies a signal the embedding does not. The reward (Section~\ref{sec:rl_mechanics}) is computed from the target's full satisfaction vector rather than its lossy $M$-dimensional embedding, so it carries semantic information the embedding has discarded, and it grades semantic agreement rather than exact-string reproduction. It is, however, a training-time signal that the model can re-access only through the embedding at generation time; it therefore cannot bypass the conditioning bottleneck, but it can recalibrate how heavily the decoder relies on a weak-but-usable signal. We accordingly hypothesise that the benefit of reinforcement learning is \emph{concentrated} on the high-variance, low-magnitude targets that cross-entropy under-serves, rather than spread uniformly across the validation set.

    Two claims follow, both tested against the embedding geometry in Part~B. The premise (\textbf{H0}) is that the decoder conditions on embedding magnitude at all --- that correctness increases with $\Vert\emb(\phi)\Vert$ on the pretrained base; were it flat, the magnitude-conditioning account would be false, itself an informative negative result. The substantive hypothesis (\textbf{H1}) is the concentration just stated: the reinforcement-learning improvement is localised to the under-conditioned, high-variance, low-norm targets. Sections~\ref{sec:geometry_analysis} and \ref{sec:rq2_comparison} make both precise and adjudicate them.

    Reinforcement learning is also motivated, more broadly, by the exposure-bias problem of teacher-forced training, for which finetuning on the model's own samples is the standard remedy; we treat this as motivation for the choice of method rather than a tested claim, since exposure bias is difficult to isolate and our hypotheses concern the kernel embedding specifically. Finally, the mutation dataset of Section~\ref{sec:finetune_data} shapes the reward landscape --- the equivalent rewrites and near-miss negations act as hard positives and hard negatives that sharpen the reward around each target --- but we do not test this directly, as the held-out set is not mutation-typed. The cross-entropy--finetuned control absorbs any such data-side effect, so the comparison between it and the reinforcement-learning models reflects the objective rather than the additional data.

    \subsection{Shared mechanics}\label{sec:rl_mechanics}
    % - One finetuning run on the 60k mutation set (NOT staged); all four finetunes start from the
    %   SAME CE-base (final pretraining checkpoint); eval on the stage-4 eval split (19,539);
    %   early stopping + best-model on eval_semantic_distance; effective batch 1024.
    % - Per item: sample one sequence (do_sample, temperature 1.0, num_beams 1) from BOS conditioned
    %   on the embedding; re-run a teacher-forced forward pass for differentiable token log-probs.
    % - Reward: R = 1 - semantic distance = 1 - normalised Hamming between the generated and target
    %   satvecs over the trace sample, clipped to [-1,1] (clip inert for valid formulae).
    % - Hybrid per-item loss: L = valid_ratio * L_RL + invalid_ratio * L_CE, where L_CE is
    %   teacher-forced CE on the ground-truth labels of the INVALID items only (a corrective anchor
    %   for samples the model cannot yet emit validly). Invalid generations are excluded from the RL
    %   term; there is no R=-1 branch. (This supersedes any NMSE-on-embeddings reward or single
    %   alpha-mixed objective from earlier notes.)
    % - Full gradient derivations -> Appendix.
    All four finetuned models start from the same pretrained base and are trained for a single, non-staged pass over the $60{,}000$-formula mutation set, with evaluation on the stage-4 evaluation split ($19{,}539$ formulae), early stopping and best-model selection on the evaluation semantic distance, and the same effective batch size of $1024$.

    For each training item, the reinforcement-learning trainers sample one formula from the policy --- ancestral sampling at temperature $1.0$, no beam search --- conditioned on the target embedding, then re-run a teacher-forced forward pass to obtain differentiable token log-probabilities. The reward is the agreement between the satisfaction vectors of the generated formula $\hat\phi$ and the target $\phi$ over the trace sample,
    \begin{equation}
        R(\hat\phi, \phi) = 1 - \frac{1}{N}\sum_{i=1}^{N}\mathbb{1}\!\left[\widehat\chi^{\,i}_{\hat\phi} \ne \widehat\chi^{\,i}_{\phi}\right],
    \end{equation}
    i.e. one minus the normalised Hamming distance, clipped to $[-1,1]$ (the clip is inert for any valid formula, whose reward lies in $[0,1]$).

    Generations that fail to parse receive no reward and are excluded from the policy-gradient term; they are instead handled by a corrective cross-entropy term on their ground-truth labels. The per-batch objective is the convex combination
    \begin{equation}
        \mathcal{L} = r_{\text{valid}}\,\mathcal{L}_{\text{RL}} + r_{\text{invalid}}\,\mathcal{L}_{\text{CE}},
    \end{equation}
    weighted by the proportions of valid and invalid generations in the batch, so the cross-entropy term acts as an anchor for samples the policy cannot yet emit validly. The per-trainer gradient derivations are deferred to the appendix.

    \subsection{Variants}\label{sec:rl_variants}
    % RB (running-baseline REINFORCE): baseline b = EMA of batch reward mean (momentum 0.9), also
    %   tracks EMA of reward^2; advantage A = (R-b)/sigma_R, detached; length-normalised sequence
    %   log-prob; L_RL = -E[A * mean log p].
    % GAE: critic = MLP (n_embd -> 256 -> 1, Tanh) on per-token last-layer hidden states; terminal
    %   reward at the last real token; GAE(gamma,lambda) advantages; actor loss -sum A log p, critic
    %   loss MSE to returns. gamma = 1 for BOTH runs; lambda in {1.0 (full Monte-Carlo), 0.9
    %   (truncated)} is the axis of contrast. Critic initialised from scratch, then a ~5-epoch
    %   actor-frozen warmup (critic-pretraining-ratio = 5/epochs). Critic LR chosen by a per-lambda
    %   sweep over {1e-2..1e-4} on (MC-)explained-variance: 5e-3 (lambda=1.0) / 1e-3 (lambda=0.9).
    % CE-finetuned control: same data + schedule, CE objective; isolates the OBJECTIVE from the
    %   extra 60k data (RL-vs-control reflects the objective).
    % Run summary:
    %   | model        | trainer            | actor LR | critic LR | epoch cap | stopped @ | wall  |
    %   | CE-finetuned | CETrainer          |  5e-6    |    --     |    50     |  50 (full)| 2h27m |
    %   | RB           | REINFORCETrainerRB |  5e-8    |    --     |    50     |    18     | 2h14m |
    %   | GAE lambda=1 | REINFORCETrainerGAE|  5e-8    |   5e-3    |    30     |   ~18     | 1h56m |
    %   | GAE lambda=.9| REINFORCETrainerGAE|  5e-8    |   1e-3    |    30     |   ~25     | 2h41m |
    %   (RB momentum 0.9; both GAE gamma=1.)
    ...
    The two \textbf{GAE} models add a learned critic --- a small multilayer perceptron ($1024 \to 256 \to 1$ with a Tanh hidden layer) reading the decoder's last-layer hidden states --- and form generalised-advantage-estimation advantages with discount $\gamma = 1$ and smoothing $\lambda \in \{1.0, 0.9\}$; $\lambda$ is the sole axis of contrast between them ($\lambda=1.0$ is full Monte-Carlo, $\lambda=0.9$ a slightly truncated, lower-variance estimate). The critic is initialised from scratch and pretrained for roughly five epochs with the actor frozen before joint training begins; its learning rate was selected per $\lambda$ by a short sweep over $\{10^{-2}, \dots, 10^{-4}\}$ on the critic's explained variance, giving $5\times10^{-3}$ for $\lambda=1.0$ and $10^{-3}$ for $\lambda=0.9$.

    The \textbf{cross-entropy--finetuned} control is trained on the same data and schedule but under the pretraining cross-entropy objective, so that the comparison between it and the reinforcement-learning models isolates the contribution of the objective from that of the additional mutation data. Table~\ref{tab:finetune} summarises the four runs.
