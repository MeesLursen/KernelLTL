...
\chapter{Experiments}\label{chap:experiments}
    ...
    \subsection{Data collecttion}\label{sec:metrics}
    ...
    The first of our experiments tested how well the trained base model was able to find the correct specification for a given semantic embedding. It establishes a baseline of feasibility for the task of mining linear temporal logic specifications from an anchor-based embedding of their semantics using the transformer architecture with cross-attention. The first part of this section will be devoted to detailing the curriculum datasets and training procedure. This is followed by the results from the two validation passes, as well as an ablation study in which we corrupted the semantic embeddings in certain ways to assess the model's reliance on this input. The latter acts as a sanity check: since the semantic embedding is the only input we provide the model at generation time, it should derive information from only this embedding \emph{and nothing else}. 
