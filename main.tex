...
\chapter{Method: Datasets, Model Architecture, and Tokenization}

    \section{Formula Sampling and Dataset Generation} \label{alg:formula_sampling_algorithm}
    ...
    \end{figure}
    In our setup we use a custom model class called \texttt{LTLModel}, which uses the \textit{HuggingFace} library's GPT2LMHeadModel with \texttt{cross-attention} enabled as a base model through which all of the forwarding and generation calls are routed with arguments adjusted to fit our setup. The GPT2LMHeadModel consists of a standard GPT2 model with a language modeling head (LM Head) on top, to produce the logits from the token embedding (see the HuggingFace \href{https://github.com/huggingface/transformers/blob/v4.57.0/src/transformers/models/gpt2/modeling_gpt2.py#L981}{GitHub repository} for details). 
    
    The essential modification to the otherwise standard GPT2 architecture is the addition of a cross-attention component within each of the GPT2 blocks Figure \ref{fig:ltl_model_architecture} describes this difference. What this allows us to do is to have the decoder attend to the semantic embedding (kernel embedding) that we inject into the \texttt{encoder\_hidden\_states}. We use the semantic embedding vector to produce \textit{Key} and \textit{Value} vectors as follows. Let $\bm{E}_{sem}$ be the semantic-embedding vector and let $\bm{W}^{cross}_k$ be the cross-attention \textit{Key}-weights matrix. Then: 
    \begin{equation}\bm{K}_{sem} = \bm{W}^{cross}_k E_{sem}\end{equation}
    
    \noindent where $\bm{K}_{sem}$ is the \textit{Key}-vector for the semantic embedding vector to which the decoder \textit{Query} attends. Similarly, let $\bm{W}^{cross}_v$ be the cross-attention \textit{Value}-weights matrix. Then:
    
    \begin{equation}\bm{V}_{sem} = \bm{W}^{cross}_v E_{sem}\end{equation}
    
    \noindent where $\bm{V}_{sem}$ is the \textit{Value}-vector from which the cross-attention update values are calculated that are added to the residual self-attention output.

    \section{Tokenization}
    

\chapter{The Linear Temporal Logic Kernel and Formula Embedding}
