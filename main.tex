...
\chapter{Experiments}\label{chap:experiments}
    ...
    The formulae used during pretraining were generated with the sampler described in Algorithm~\ref{alg:formula_sampling_algorithm}, in four cumulative rounds of increasing maximum depth. Each round extended the set produced by the previous one and sampled under its own leaf probability range $p_{\text{leaf}}$, listed in Table~\ref{tab:curriculum}. The lower ranges at the later rounds counteract the sampler's bias toward shallow trees as the depth bound grows. The result is a single pool of $890{,}493$ formulae of depth at most $5$, distributed over depths $0$ through $5$ as $22{,}612$, $26{,}041$, $50{,}385$, $110{,}119$, $219{,}448$, and $461{,}888$ formulae respectively. The training set of stage $i$ is this pool restricted to depth at most $i+1$, so that the four stages are \emph{nested}: every formula seen at one stage is seen again at all later stages, and each stage adds only the formulae of the next depth. Because each round draws approximately as many formulae as all previous rounds combined, this restriction approximately doubles the training set at every stage.
