...
\chapter{Method: Datasets, Model Architecture, and Tokenization}
    ...
    The single departure from the standard GPT-2 architecture is the insertion of a Cross Attention block within each Transformer Block. In the vanilla configuration, each Transformer Block applies a Layer Normalization followed by Masked Self Attention and a Residual Connection, then a second Layer Normalization followed by a Multi-Layer Perceptron and a final Residual Connection. Enabling cross-attention inserts a third sublayer between the first Residual Connection and the MLP: a Layer Normalization, Cross Attention, and Residual Connection. This Cross Attention block allows the decoder's hidden states to use the \emph{Semantic Embedding Hidden States} $\bm{E}_{sem} \in \mathbb{R}^M$ as an external conditioning signal, which encodes the target formula's satisfaction behavior. Rather than supplying the conditioning signal once at the input as a prefix, this mechanism makes it available to the decoder at every layer and at every decoding step.
    
    The Cross Attention block operates as follows. Let $\bm{h}_t \in \mathbb{R}^d$ denote the decoder hidden state at position $t$ after the first Residual Connection. The \emph{Query}, \emph{Key}, and \emph{Value} vectors are produced by three separate learned projections:
    \begin{align}
        \bm{Q}_t     &= \bm{W}^{cross}_q \bm{h}_t \\
        \bm{K}_{sem} &= \bm{W}^{cross}_k \bm{E}_{sem} \\
        \bm{V}_{sem} &= \bm{W}^{cross}_v \bm{E}_{sem}
    \end{align}
    where $\bm{W}^{cross}_q \in \mathbb{R}^{d_k \times d}$ maps the decoder state to a query, and $\bm{W}^{cross}_k, \bm{W}^{cross}_v \in \mathbb{R}^{d_k \times M}$ project the Semantic Embedding Hidden States into a key and a value. The cross-attention weight at position $t$ is then:
    \begin{equation}
        a_t = \mathrm{softmax}\!\left(\frac{\bm{Q}_t \cdot \bm{K}_{sem}}{\sqrt{d_k}}\right)
    \end{equation}
    and the resulting output $a_t\,\bm{V}_{sem}$, projected back to the dimension of the decoder hidden states, is passed through the Residual Connection into the Layer Normalization preceding the Multi-Layer Perceptron. Because $\bm{E}_{sem}$ is presented as a single conditioning token, the softmax is computed over a single scalar and reduces to the trivial weight $a_t = 1$ for every $t$. Each decoder position therefore receives the same additive update $\bm{V}_{sem}$ at every Transformer Block, and the strength of this conditioning is governed entirely by the magnitude of $\bm{V}_{sem}$, which is bounded by $\Vert\bm{W}^{cross}_v\Vert\,\Vert\bm{E}_{sem}\Vert$. We return to the implications of this property in Section~\ref{sec:the_embedding_kernel}, where it motivates our choice of the covariance kernel over normalized alternatives.
