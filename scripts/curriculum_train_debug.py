...
def parse_args() -> argparse.Namespace:
    ...
    train_group.add_argument("--logging-steps", type=_positive_int, default=None)
    train_group.add_argument("--eval-steps", type=_positive_int, default=None)
    train_group.add_argument("--save-steps", type=_positive_int, default=None)
...
def main() -> None:
    ...
    if args.model_load_dir:
        print(f"\n{'='*60}")
        print(f"DEBUG: Inspecting checkpoint at {args.model_load_dir}")
        print(f"{'='*60}")
        
        # List directory contents
        print("\nDirectory contents:")
        for f in sorted(os.listdir(args.model_load_dir)):
            fpath = os.path.join(args.model_load_dir, f)
            size = os.path.getsize(fpath) if os.path.isfile(fpath) else "[dir]"
            print(f"  {f}: {size}")
        
        # Check for weight files and inspect keys
        weights_path = None
        candidate = os.path.join(args.model_load_dir, "pytorch_model.bin")
        ...
        if weights_path:
            ...
            base_prefixed = sum(1 for k in keys if k.startswith("base."))
            print(f"\nKeys with 'base.' prefix: {base_prefixed}/{len(keys)}")
        else:
