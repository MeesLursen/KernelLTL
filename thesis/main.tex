...
\chapter{Experiments}\label{chap:experiments}
    ...
    Each model was evaluated on the validation dataset by having it generate responses to the semantic embeddings of the formulae in the dataset. We examined the models under two regimes: one deterministic greedy pass that asked the model to generate one sequence per embedding, and one stochastic pass that asked the model to generate five sequences per embedding. See Table \ref{tab:validation_generation_settings} for an overview of the the specific generation argument used during both of the passes. At generation time of both passes, we collected several metrics. We specify what metrics we collected in the section. For each model, we compiled the data gathered during both passes into separate files. 
    
    After all of the data was collected, we marked each of the tautological or contradictory targets in the datasets. These formulae have the trivial \textit{all-true} and \textit{all-false} satisfaction vectors respectively and thus the covariance kernel $k^\cov$ maps them both to the zero embedding, as shown in Chapter \ref{chap:kernel}). As was discussed before, these targets post a meaningless question in the context of specification mining. Therefore, we opted to flag each of these items in the dataset, so they could be disregarded during the upcoming analyses. In total our validation dataset contained $1{,}545$ trivial targets, omitting them leaves our dataset with $18{,}455$ non-trivial formulae against which all of the statistics in this chapter were computed.
