...
\chapter{Experiments}\label{chap:experiments}
    ...
    All reported results in this chapter were computed on a held-out validation dataset. The dataset consists of $20{,}000$ formulae, split evenly into subsets of $5{,}000$ formulae for each of the depths $2$ through $5$. The construction consisted of repeatedly sampling a large batch of formulae using Algorithm \ref{alg:formula_sampling_algorithm}, after which every formulae that was syntactically identical to any formula already seen in any of the datasets used to train, finetune, or select the models were discarded. The remaining formulae were then added to the dataset if the total number of formulae of their respective depth had not yet reached the cap of $5{,}000$. This process was repeated until all depths were fully saturated. 
    ...
    Each model was evaluated on the validation dataset by having it generate responses to the semantic embeddings of the formulae in the dataset. We examined the models under two regimes: one greedy pass where the model generated one sequence per embedding, and one stochastic pass where the model to generated five sequences per embedding. See Table \ref{tab:validation_generation_settings} for an overview of the specific generation arguments used during both passes. At generation time of both passes, we collected several metrics. We specify what metrics were collected in the following section section. 
    
    For each model, we compiled all of the data gathered during both passes into separate files. Afterwards, we marked each of the model's responses to a tautological or contradictory target in the validation dataset with a \texttt{is\_trivial} flag. These formulae have satisfaction vectors consisting of \textit{all-true} and \textit{all-false} truth values respectively and thus are mapped to the zero embedding by the covariance kernel $k^\cov$, as shown in Chapter \ref{chap:kernel}. As was discussed there, these targets pose a meaningless question in the context of specification mining. Therefore, we flagged each of these items in the dataset, so they could be disregarded during the statistical analyses that inform the upcoming analyses. In total our validation dataset contained $1{,}545$ trivial targets and omitting the trivial formulae left our dataset with $18{,}455$ non-trivial formulae against which all of the statistics in this chapter were computed.
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{lcc}
            ...
            Selection rule                  & argmax            & multinomial \\
    ...
    \NEW{Comparisons are paired by target against a reference model. We treat the effect size---matched-pairs rank-biserial correlation for continuous metrics, the difference in proportions for binary ones---together with its $95\%$ bootstrap confidence interval as the headline, and a Benjamini--Hochberg-adjusted $p$-value as a backstop; with $n \approx 18{,}000$ statistical significance is cheap, so magnitude rather than significance carries the inference, and we set no fixed practical-significance threshold. We run the comparison under two references: against the cross-entropy base, each row is a finetuning's \emph{full} effect (objective plus the additional data together); against the cross-entropy-finetuned control, the reinforcement-learning rows isolate the contribution of the \emph{objective} from that of the data. Absolute per-model descriptives are reported alongside the relative tables so that a modest or null effect remains legible.}
    
    \NEW{\section{Part I --- Feasibility: can the model recover specifications?}\label{sec:rq1_feasibility}}
    
    \NEW{The first question is whether the pipeline works at all: conditioned only on the kernel embedding of a target's behaviour, can the decoder produce a formula that reproduces that behaviour? Part~I specifies how the baseline model was built---its pretraining curriculum and hyperparameter search---and how its competence is measured. The cross-entropy model that results is the baseline that Parts~II and~III respectively dissect and attempt to improve.}
