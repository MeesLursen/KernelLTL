...
To our knowledge this is the first time that this specific transformer based solution for recovering LTL formulae from an embedding of their semantics has been been tried. Therefore, the first of our experiments starts with training a base model on the GPT standard cross-entropy loss function to assess whether our solution holds up in practice. As a whole, the experiments performed in this chapter are designed to answer a sequence of consecutive research questions. \textbf{Experiment 1} investigates how well our trained base model recovers the formula that produced the semantic embedding we supply it with during generation, as well as how well our model is able to discover multiple syntactical representations of the same semantics. Next, \textbf{Experiment 2} asks whether the failure cases of our model exhibit any structurally identifiable patterns. Finally, \textbf{Experiment 3} explores what the effects are of finetuning with a reinforcement learning loss function on the generation policy of our base model. It investigates the broad effects of reinforcement learning based finetuning on general formula recovery performance, as well as more narrow questions about its effects on the problematic areas identified in Experiment 2 and the range of distinct syntactic forms that the model is able to identify. 

Due to the consecutive nature of our experiments, this chapter is constructed by first introducing the shared machinery that each of the experiments makes use of, followed by a self-contained \textit{methodology, results, discussion} section for each one of the experiments.


\section{Shared experimental apparatus}\label{sec:shared_apparatus}
Before moving on to address each of these questions in order, we will be laying out the shared mechanics that each of the experiments relied on. In section \S~\ref{sec:kernel_instantiation} we will be describing the precise instantiation of our kernel used to produce the semantic embeddings for the formulae in our datasets. Then, in \S~\ref{sec:validation_dataset} we will be describing the construction of our dataset used to validate each of the models we trained during the experiments. In \S~\ref{sec:metrics} we describe what metrics were collected during the validation process and in \S~\ref{sec:stats_framework} we describe what statistical conventions our back up our analyses.  
...
The trace sample was fixed at $N = 500{,}000$, drawn from the mixture distribution of Section~\ref{sec:trace_sampling_details}. The mixture consisted of $70\%$ high-variance, i.i.d.\ Bernoulli$(0.5)$ traces and the remaining $30\%$ were the low-variance, temporally-correlated Markov traces with flip probability $p_{\text{flip}}=0.1$. The size of the trace sample $N$ was taken above he lower bound of Equation~\eqref{eq:n_min}. Instantiated at $M = 1024$, tolerance $\eta = 0.005$, and confidence $\delta = 0.01$, the bound requires a strict minimum of $N \ge 258{,}469$ traces. We landed on $N = 500{,}000$, approximately twice this minimum. At this sample size, with probability at least $99\%$, each of the empirical base- and joint-rate values involved in calculating the embedding vector $\emb(\phi)$ is within $\approx 0.0036$ and each kernel coordinate $k^{\cov}(\phi, \psi_j)$ within $\approx 0.011$ of their respective true values (Appendix~\ref{app:tolerance_derivation}). With the range of the kernel values being $[-\tfrac14, \tfrac14]$ of $k^{\cov}$, this amounts to a deviation of (at most) approximately $2\%$.

\NEW{\subsection{Held-out validation protocol}\label{sec:validation_dataset}}
\NEW{All reported results are computed on a held-out validation set of $20{,}000$ formulae, depth-stratified at $5{,}000$ each for depths $2$ through $5$, sampled from the kernel and made disjoint from every training and finetuning formula. Because its maximum depth equals the maximum training depth, the set measures generalisation to \emph{unseen instances within trained depths}, not extrapolation to deeper formulae; we make no depth-extrapolation claim. Each model is decoded in two regimes: a deterministic greedy pass and a top-$5$ sampling pass at temperature $1.0$. Tautology and contradiction targets, whose satisfaction vector is all-true or all-false, are mapped by the covariance kernel to the zero embedding and are therefore un-conditionable by construction; we drop all $1{,}545$ of them suite-wide, so every reported figure is over $n = 18{,}455$ targets.}

\NEW{\subsection{Metrics}\label{sec:metrics}}
