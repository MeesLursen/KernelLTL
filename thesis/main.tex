...
% Preamble (once): \usepackage{xcolor}\newcommand{\NEW}[1]{\textcolor{blue}{#1}}
% \NEW{...} marks text drafted by me (unvetted); all unmarked prose is your verbatim, relocated text.
...
Due to the consecutive nature of our experiments, this chapter is constructed by first introducing the shared machinery that each of the experiments makes use of, followed by a self-contained \textit{methodology}, \textit{results}, and \textit{discussion} section for each of the experiments.


\section{Shared experimental apparatus}\label{sec:shared_apparatus}
Before moving on to address each of these questions in order, we will be laying out the shared mechanics that each of the experiments relied on. In \S~\ref{sec:kernel_instantiation} we will be describing the precise instantiation of our kernel used to produce the semantic embeddings for the formulae in our datasets. Then, in \S~\ref{sec:model_validation} we will be describing the construction of our dataset used to validate each of the models we trained during the experiments. In \S~\ref{sec:metrics} we describe what metrics were collected during the validation process and in \S~\ref{sec:stats_framework} we describe what statistical conventions back up our analyses.  
...
All reported results in this chapter were computed on a held-out validation dataset. The dataset consists of $20{,}000$ formulae, split evenly into subsets of $5{,}000$ formulae for each of the depths $2$ through $5$. The sampling procedure consisted of sampling large batches of formulae using Algorithm \ref{alg:formula_sampling_algorithm}, after which each of the formulae in this batch that were already seen in any of the training or evaluation datasets used to train, finetune, or select the models were discarded. The remaining formulae in this batch were added to the dataset if the total number of formulae of their respective depth has not yet reached the cap of $5{,}000$. This process was repeated until all depths were fully saturated. 

As will become clear during the description of the datasets used to train and evaluate the models in the upcoming experiments, our validation dataset only contains formulae of depths that the model has been trained on. Thus, any claims about \emph{generalization to unseen data} put forward in the analysis of the validation results must be interpreted as \emph{generalization to formulae within the training depth distribution}, we make no claims about extrapolation beyond these depths.

Each model is decoded in two regimes: a deterministic greedy pass and a top-$5$ sampling pass at temperature $1.0$. Tautology and contradiction targets, whose satisfaction vector is all-true or all-false, are mapped by the covariance kernel to the zero embedding and are therefore un-conditionable by construction; we drop all $1{,}545$ of them suite-wide, so every reported figure is over $n = 18{,}455$ targets.
...
\NEW{Part~I establishes that the base model works, and Part~II that it fails on a geometrically and symbolically identifiable cell---a usable but weak conditioning signal it under-exploits. Two levers are available to address this: the additional, mutation-augmented data on which the base is finetuned, and the training objective itself. We pull both and separate them with a control. All four finetunes start from the same base and see the same data; the cross-entropy-finetuned model changes nothing but its exposure to that data, while the three reinforcement-learning models additionally replace teacher-forced cross-entropy---which supervises only on ground-truth prefixes and rewards only exact reproduction of a single reference string, inheriting the exposure bias of \S\ref{sec:prelim_ce}---with a reward computed from the model's own samples (\S\ref{sec:rl_mechanics}). Comparing each finetune against the cross-entropy base measures a finetuning's full effect; comparing the reinforcement models against the cross-entropy control isolates the contribution of the objective from that of the data.}
