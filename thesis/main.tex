...
\chapter{Experiment Design}\label{chap:experiment_design}
    ...
    The pretraining phase fixes the base model that all four finetuned variants inherit. Its objective is the teacher-forced cross-entropy of Section~\ref{sec:prelim_ce}: at each position the model is supervised to maximize the log-probability of the next ground-truth token of the canonical string $\mathrm{str}(\phi)$, conditioned on the target formula's semantic embedding through the cross-attention mechanism of Chapter~\ref{chap:kernel}. Minimizing this objective is maximum-likelihood estimation of the map from an embedding to the single canonical syntax that realizes it; what results is, in effect, a conditional language model over Linear Temporal Logic whose conditioning channel is the kernel embedding.
    
    The data is presented as the four-stage, depth-staged curriculum of Section~\ref{sec:curriculum_data}. Each stage extends the previous one with formulae a level deeper and roughly doubles the dataset, and each resumes from the weights and optimizer state of the last, so that the model is confronted with progressively deeper temporal structure only once it has converged on the shallower fragment out of which that structure is composed.
    ...
    For each training item, the reinforcement-learning trainers sample one formula from the policy --- ancestral sampling at temperature $1.0$, no beam search --- conditioned on the target embedding, then re-run a teacher-forced forward pass (\S\ref{sec:prelim_ce}) to obtain differentiable token log-probabilities. The reward is the agreement between the satisfaction vectors of the generated formula $\hat\phi$ and the target $\phi$ over the trace sample,
    ...
    The two \textbf{GAE} models add a learned critic --- a small multilayer perceptron ($1024 \to 256 \to 1$ with a Tanh hidden layer) reading the decoder's last-layer hidden states, the per-token value that lets the estimator localise credit (\S\ref{sec:objective_contrast}) --- and form generalised-advantage-estimation advantages with discount $\gamma = 1$ and smoothing $\lambda \in \{1.0, 0.9\}$; $\lambda$ is the sole axis of contrast between them ($\lambda=1.0$ is full Monte-Carlo, $\lambda=0.9$ a slightly truncated, lower-variance estimate). The critic is initialised from scratch and pretrained for roughly five epochs with the actor frozen before joint training begins; its learning rate was selected per $\lambda$ by a short sweep over $\{10^{-2}, \dots, 10^{-4}\}$ on the critic's explained variance, giving $5\times10^{-3}$ for $\lambda=1.0$ and $10^{-3}$ for $\lambda=0.9$.
